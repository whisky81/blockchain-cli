...
import { BLOCKCHAIN_ENTIRE_CHAIN_PROTOCOL, BLOCKCHAIN_LATEST_BLOCK_PROTOCOL } from "./Node.js";
...
export function helpPrompt() {
    ...
    console.log(' '.repeat(4) + chalk.cyanBright('Describe the event system in detail (used in `.log.event.<(list|discovery|connect|disconnect|self-update)>` command)'));
    console.table([
        { 'Event name': 'discovery', 'Description': 'Triggered when a new peer is discovered via any discovery mechanism.' },
        { 'Event name': 'connect', 'Description': 'Triggered when a connection to a new peer is established' },
        { 'Event name': 'disconnect', 'Description': 'Triggered when the last connection to a peer is closed' },
        { 'Event name': 'self-update', 'Description': "Emitted when the current node's own peer record changes (e.g., new address, protocol handler registered)." }
    ]);

    console.log(' '.repeat(4) + chalk.cyanBright('Describe the protocols in system'));
    console.table([
        { 'Protocol': BLOCKCHAIN_LATEST_BLOCK_PROTOCOL, 'Description': 'Protocol used to send the latest block to a specified peer' },
        { 'Protocol': BLOCKCHAIN_ENTIRE_CHAIN_PROTOCOL, 'Description': 'Protocol used to send the entire chain to a specified peer' }
    ]);
    console.log(`${chalk.redBright('Note that')} the ${chalk.cyanBright('.log.protocol.latest-block')} or ${chalk.cyanBright('.log.protocol.entire-chain')} command only displays information about the latest blocks and the entire chains received from other peers. It does not store information about the blocks or chains that have been sent`);
